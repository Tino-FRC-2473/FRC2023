...
public class ArmFSM {
	...
	//20 inches
	private static double armEncoderMaxLengthRotations = 180;
	...
	private static final double ARM_ENCODER_HIGH_FORWARD_CONE_ROTATIONS = 21
		* ENCODER_TICKS_TO_ARM_LENGTH_INCHES_CONSTANT;
	//12 inches
	private static final double ARM_ENCODER_HIGH_BACKWARD_ROTATIONS = 28
		* ENCODER_TICKS_TO_ARM_LENGTH_INCHES_CONSTANT;
	//3 inches
	...
	public void reset() {
		...
		if (pivotMotor.getEncoder().getPosition() > ARM_ENCODER_VERTICAL_ANGLE_ROTATIONS) {
			armEncoderMaxLengthRotations = 180 / Math.abs(Math.cos(Math.toRadians(
				-pivotMotor.getEncoder().getPosition() / ENCODER_TICKS_TO_ARM_ANGLE_DEGREES_CONSTANT)));
		} else {
			armEncoderMaxLengthRotations = 198 / Math.abs(Math.cos(Math.toRadians(
				-pivotMotor.getEncoder().getPosition() / ENCODER_TICKS_TO_ARM_ANGLE_DEGREES_CONSTANT)));
		}
